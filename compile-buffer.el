...
(defun CB-run-buffer-as-latex()
  ...
  (let* (
	...
	(CB-show-and-set-buffer-height buffer CB-running-window-height)
	(shell-command
	 (format "%s %s.pdf >/dev/null 2>&1 &" CB-pdf-viewer base)
	 buffer buffer
	 )
	)
  )
(defun CB-try-kill-buffer-process(buffer)
  (if (get-buffer-process buffer)
	  (if (y-or-n-p "There is a process running. Kill it?")
...
(defun CB-compile-buffer()
  ...
  (let* ((filetype (CB-get-buffer-filetype))
	...
	(CB-try-kill-buffer-process CB-compiling-buffer)
...
CB-compile-buffer first."
  ...
  (let* (
	...
	(CB-try-kill-buffer-process CB-running-buffer)
